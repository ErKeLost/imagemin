...
impl ImageInfo {
    fn new(prev_path: &String) -> Self {
        Self {
            prev: Imageh::new(prev_path),
            later: Imageh::default(),
            img_type: "".into(),
        }
    }
}
...
pub fn run(config: &Args) -> Result<(), Box<dyn Error>> {
    ...
    println!("{:?} 当前时间", started);
    let path = Path::new(&config.path);
    println!("{:?} 图片文件地址", &config.path);
...
fn print_table(list: &mut Vec<ImageInfo>) {
    ...
    for v in list.iter() {
        let row = Row::from([
            v.prev.path.clone(),
            fomramt_size(v.prev.size),
            v.later.path.clone(),
            fomramt_size(v.later.size),
        ]);
        rows.push(row);
    }
...
fn fomramt_size(filesize: u64) -> String {
    if filesize < 1024 {
